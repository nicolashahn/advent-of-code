...
def interface(prog_ref):
    if sys.version[0] == "2":
        input = raw_input
    while True:
        raw = input()
        # save with: "SAVE <N>" where <N> is slot number
        if "SAVE" in raw:
            slot = raw.split()[1]
            with open("save-{}.txt".format(slot), "w") as savefile:
                savefile.write(str(prog_ref)[1:-1])
...
def p1(prog):
    outbuf = []
    p1prog = prog[:]
    outgen = execute(p1prog, interface(p1prog))
    for out in outgen:
        if chr(out) == "\n":
            print("".join([chr(o) for o in outbuf]))
            outbuf = []
        else:
            outbuf.append(out)
